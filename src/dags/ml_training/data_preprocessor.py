...
#             # local
#             # --- CRITICAL FIX: Add nrows=1000000 ---
#             logger.info("Loading transactions_data (optimized)...")
#             self.transactions_df = pd.read_csv(
#                 self.data_config['transactions_data'], 
#                 dtype=dtypes,
#                 parse_dates=['date'],
#                 nrows=1000000  # <--- LIMITS LOAD TO 1 MILLION ROWS TO PREVENT CRASH
#             )
#             logger.info(f"Loaded transactions_data: {len(self.transactions_df)} rows")
#             # ===================================
#             # Randomly sample 10% of data to prevent Docker Memory Crash (Zombie Task)
#             # 1.3 million rows is still plenty for Isolation Forest
#             # logger.info(f"Original dataset size: {len(self.transactions_df)}")
#             # self.transactions_df = self.transactions_df.sample(frac=0.1, random_state=42)
#             # logger.info(f"Downsampled dataset size: {len(self.transactions_df)}")
#             # logger.info(f"Loaded transactions_data: {len(self.transactions_df)} rows")
#             # ===================================
...
Data Preprocessor for Fraud Detection System
Loads and processes data from the data/ folder
FIXED VERSION - Resolves pandas FutureWarnings and dtype issues
"""
...
class DataPreprocessor:
    ...
    def merge_data(self):
        ...
        # Add fraud labels
        labels_df_temp = self.labels_df.reset_index()
        labels_df_temp = labels_df_temp.rename(columns={'index': 'id', 'target': 'is_fraud'})
        labels_df_temp['id'] = labels_df_temp['id'].astype('Int64')
        df['id'] = df['id'].astype('Int64')
        df = pd.merge(df, labels_df_temp, on='id', how='left')
        df['is_fraud'] = df['is_fraud'].apply(lambda x: 1 if x == 'Yes' else 0)
    ...
    def preprocess(self):
        """Apply all preprocessing steps - FIXED to avoid FutureWarnings"""
        logger.info("Preprocessing data...")
        
        # FIX: Create explicit copy to avoid SettingWithCopyWarning
        df = self.merged_df.copy()
        
        # FIX: Handle missing values properly without chained assignment
        ...
        # Drop rows with critical missing values
        df = df.dropna(subset=['amount', 'use_chip', 'merchant_id', 'mcc', 'description', 'merchant_city'])
        ...
        # FIX: Reassign to self.merged_df instead of modifying in place
        self.merged_df = df
        logger.info("Basic preprocessing completed")
    ...
    def engineer_features(self):
        """Create engineered features - FIXED to avoid FutureWarnings"""
        logger.info("Engineering features...")
        
        # FIX: Create explicit copy
        df = self.merged_df.copy()
        ...
        # FIX: Direct assignment instead of inplace
        ...
        rolling_mean_series = df.groupby('client_id').rolling(window='3D', on='date', closed='left')['amount'].mean()
        rolling_mean_df = rolling_mean_series.reset_index()
        rolling_mean_df = rolling_mean_df.rename(columns={'amount': 'rolling_mean_3day'})
        
        rolling_std_series = df.groupby('client_id').rolling(window='3D', on='date', closed='left')['amount'].std()
        rolling_std_df = rolling_std_series.reset_index()
        rolling_std_df = rolling_std_df.rename(columns={'amount': 'rolling_std_3day'})
        ...
        # FIX: Reassign instead of modifying in place
        self.merged_df = df
        logger.info("Feature engineering completed")
